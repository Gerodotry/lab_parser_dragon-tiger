...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    TOK_END = 287,                 // "end"
    TOK_BREAK = 288,               // "break"
    TOK_FUNCTION = 289,            // "function"
    TOK_VAR = 290,                 // "var"
    TOK_UMINUS = 291,              // "uminus"
    TOK_ID = 292,                  // "id"
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_ifExpr = 55,                           // ifExpr
        S_whileExpr = 56,                        // whileExpr
        S_forExpr = 57,                          // forExpr
        S_breakExpr = 58,                        // breakExpr
        S_letExpr = 59,                          // letExpr
        S_seqExpr = 60,                          // seqExpr
        S_exprs = 61,                            // exprs
        S_nonemptyexprs = 62,                    // nonemptyexprs
        S_arguments = 63,                        // arguments
        S_nonemptyarguments = 64,                // nonemptyarguments
        S_params = 65,                           // params
        S_nonemptyparams = 66,                   // nonemptyparams
        S_decls = 67,                            // decls
        S_param = 68,                            // param
        S_typeannotation = 69                    // typeannotation
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_typeannotation: // typeannotation
        value.move< boost::optional<Symbol> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, Symbol&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, boost::optional<Symbol>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
...
switch (yykind)
    {
      ...
      {
        return tiger_parser::symbol_name (this->kind ());
      }
...
#if 201103L <= YY_CPLUSPLUS
    /// Non copyable.
    tiger_parser (const tiger_parser&) = delete;
    /// Non copyable.
    tiger_parser& operator= (const tiger_parser&) = delete;
#endif
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_ID (Symbol v, location_type l)
      {
      ...
      static
      symbol_type
      make_STRING (Symbol v, location_type l)
      {
    ...
    {
    public:
      context (const tiger_parser& yyparser, const symbol_type& yyla);
  ...
  tiger_parser::yytranslate_ (int t) YY_NOEXCEPT
  {
  ...
  // basic_symbol.
  template <typename Base>
  tiger_parser::basic_symbol<Base>::basic_symbol (const basic_symbol& that)
    : Base (that)
    , value ()
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_typeannotation: // typeannotation
        value.copy< boost::optional<Symbol> > (YY_MOVE (that.value));
        break;
  ...
  template <typename Base>
  tiger_parser::symbol_kind_type
  tiger_parser::basic_symbol<Base>::type_get () const YY_NOEXCEPT
  {
    return this->kind ();
  }
  ...
  template <typename Base>
  bool
  tiger_parser::basic_symbol<Base>::empty () const YY_NOEXCEPT
  {
  ...
  tiger_parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_typeannotation: // typeannotation
        value.move< boost::optional<Symbol> > (YY_MOVE (s.value));
        break;
  ...
  // by_kind.
  inline
  tiger_parser::by_kind::by_kind () YY_NOEXCEPT
    : kind_ (symbol_kind::S_YYEMPTY)
  {}

#if 201103L <= YY_CPLUSPLUS
  inline
  tiger_parser::by_kind::by_kind (by_kind&& that) YY_NOEXCEPT
    : kind_ (that.kind_)
  {
    that.clear ();
  }
  ...
  inline
  tiger_parser::by_kind::by_kind (const by_kind& that) YY_NOEXCEPT
    : kind_ (that.kind_)
  {}

  inline
  tiger_parser::by_kind::by_kind (token_kind_type t) YY_NOEXCEPT
  ...
  inline
  void
  tiger_parser::by_kind::clear () YY_NOEXCEPT
  {
    kind_ = symbol_kind::S_YYEMPTY;
  }
  ...
  inline
  void
  tiger_parser::by_kind::move (by_kind& that)
  {
    kind_ = that.kind_;
    that.clear ();
  }
  ...
  inline
  tiger_parser::symbol_kind_type
  tiger_parser::by_kind::kind () const YY_NOEXCEPT
  {
    return kind_;
  }
  ...
  inline
  tiger_parser::symbol_kind_type
  tiger_parser::by_kind::type_get () const YY_NOEXCEPT
  {
    return this->kind ();
  }
